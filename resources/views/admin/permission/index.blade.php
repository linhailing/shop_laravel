    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group ">
                ...
                <a class="layui-btn layui-btn-sm" href="{{ route('admin.permission.store') }}">添 加</a>
    ...
    <script>
        layui.use(['layer','table','form'],function () {
            ...
            table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'del'){
                    layer.confirm('确认删除吗？', function(index){
                        $.post("{{ route('admin.permission.del') }}",{_method:'delete',id:data.id},function (result) {
                            if (result.code==0){
                    ...
                    location.href = '/admin/permission/store?id='+data.id;
            ...
            $("#listDelete").click(function () {
                ...
                if (ids.length>0){
                    layer.confirm('确认删除吗？', function(index){
                        $.post("{{ route('admin.permission.del') }}",{_method:'delete',ids:ids},function (result) {
                        if (result.code==0){
                            dataTable.reload()
                        }
                        layer.close(index);
